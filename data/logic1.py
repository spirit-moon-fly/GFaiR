...
def unify(form1, form2, subst):
    if form1.isa(Variable):
        return unifyTerms(form1, form2, subst)
    if form1.isa(Constant):
        return unifyTerms(form1, form2, subst)
    if form1.isa(Atom):
...
class ToCNFRule(UnaryRule):
    ...
    def applyRule(self, form):
        ...
        def updateSubst(subst, var, exist=True):
            if exist:
                self.varCounts['$x'] += 1
                newVar = Variable('$x' + str(self.varCounts['$x']))
            else:
                self.varCounts['$y'] += 1
                newVar = Variable('$y' + str(self.varCounts['$y']))
        ...
        def standardizeVariables(form, subst):
            ...
            if form.isa(Forall):
                newSubst = updateSubst(subst, form.var,False)
        ...
        def skolemize(form, subst, scope): 
            if form.isa(Variable):
                return subst.get(form, form)
            if form.isa(Constant):
                return form
            if form.isa(Atom):
                return Atom(*[form.name] + [skolemize(arg, subst, scope) for arg in form.args])
            if form.isa(Not):
                return Not(skolemize(form.arg, subst, scope))
            if form.isa(And):
                return And(skolemize(form.arg1, subst, scope), skolemize(form.arg2, subst, scope))
            if form.isa(Or):
                return Or(skolemize(form.arg1, subst, scope), skolemize(form.arg2, subst, scope))
            if form.isa(Exists):
...
class Derivation:
    def __init__(self, form, children, cost, derived,num=-1):
        self.form = form
...
class KBResponse:
    ...
    # status: one of the ENTAILMENT, NEURAL, CONTRADICTION
...
def showKBResponse(response, verbose=1):
    if isinstance(response, KBResponse):
        ...
        print(('Maybe: %s' % rstr([obj for obj, status in items if status == NEURAL])))
        print(('No: %s' % rstr([obj for obj, status in items if status == CONTRADICTION])))

def equal_fol(items1,items2):
    def unify1(form1, form2, subst):
        if form1.isa(Variable): return unifyTerms1(form1, form2, subst)
        if form1.isa(Constant): return unifyTerms1(form1, form2, subst)
        if form1.isa(Atom):
            return form2.isa(Atom) and form1.name == form2.name and len(form1.args) == len(form2.args) and \
                all(unify1(form1.args[i], form2.args[i], subst) for i in range(len(form1.args)))
        if form1.isa(Not):
            return form2.isa(Not) and unify1(form1.arg, form2.arg, subst)
        if form1.isa(And):
            return form2.isa(And) and unify1(form1.arg1, form2.arg1, subst) and unify1(form1.arg2, form2.arg2, subst)
        if form1.isa(Or):
            return form2.isa(Or) and unify1(form1.arg1, form2.arg1, subst) and unify1(form1.arg2, form2.arg2, subst)
        raise Exception('Unhandled: %s' % form1)

    # Follow multiple links to get to x
    def getSubst1(subst, x):
        while True:
            y = subst.get(x)
            if y == None:
                return x
    ...
    def unifyTerms1(a, b, subst):
        a = getSubst1(subst, a)
        b = getSubst1(subst, b)
        if a == b: return True
        if a.isa(Variable) and b.isa(Variable):
            subst[a] = b
        else:
            return False
        return True

    if items1 is AtomFalse and items2 is AtomFalse:
        return True
    if items1 is AtomFalse and items2 is not AtomFalse:
        return False
    if items2 is AtomFalse and items1 is not AtomFalse:
        return False
    items1 = flattenOr(items1)
    items2 = flattenOr(items2)
    for i, item1 in enumerate(items1):
        for j, item2 in enumerate(items2):
            subst = {}
            if unify1(item1, item2, subst):
                newItems1 = [applySubst(item, subst) for item in items1]
                newItems2 = [applySubst(item, subst) for item in items2]
                l1=[0 for _ in items1]
                l2=[0 for _ in items2]
                for k in range(len(newItems1)):
                    for h in range(len(newItems2)):
                        if newItems2[h]==newItems1[k]:
                            l1[k]=1
                            l2[h]=1
                if 0 not in l1 and 0 not in l2:
                    return True
    l1 = [0 for _ in items1]
    l2 = [0 for _ in items2]
    for k in range(len(items1)):
        for h in range(len(items2)):
            if items2[h] == items1[k]:
                l1[k] = 1
                l2[h] = 1
    if 0 not in l1 and 0 not in l2:
        return True
...
class KnowledgeBase:
    ...
    def tell(self, form,num,getprocess=False):
        return self.query(form, modify=True,num=num,getprocess=getprocess)
    ...
    def ask(self, form, num, getprocess=False):
        return self.query(form, modify=False,num=num,getprocess=getprocess)
    ...
    def getstandard(self):
        return self.standardizationRule

    def getderivations(self):
        l=['' for _ in range(len(self.derivations))]
        #print(len(l))
        for key in self.derivations.keys():
            #print(self.derivations[key].num,key)
            l[self.derivations[key].num]=key
        #print()
        return l

    def query(self, form, modify, num=-1, getprocess=False):
        #print 'QUERY', form
        ...
        if modify is True:
            if getprocess:
                if form not in self.derivations.keys():
                    deriv = Derivation(form=form, children=[], cost=0, derived=False, num=num)
                    deriv.permanent = True
                    self.derivations[deriv.form] = deriv
                    num+=1
                return num, KBResponse(query='', modify=modify, status=NEURAL, trueModel=trueModel, falseModel=falseModel)

            form_tmp = self.standardize(form)
            for f in form_tmp:
                if f == AtomTrue or f in self.derivations.keys():
                    continue
                #print(num,f)
                deriv = Derivation(form=f, children=[], cost=0, derived=False,num=num)
                num+=1
                deriv.permanent=True
                self.derivations[deriv.form] = deriv
            return num,KBResponse(query = '', modify = modify, status = NEURAL, trueModel = trueModel, falseModel = falseModel)
        # Add Not(form)
        if getprocess:
            f = self.addAxiom(form, num=num, getprocess=getprocess)
            if f is False:
                ...
                self.removeTemporary()
                status = NEURAL
            return f,KBResponse(query = '', modify = modify, status = status, trueModel = trueModel, falseModel = falseModel)

        f=self.addAxiom(Not(form),num=num)
        if f is False:
            #self.removeTemporary()
            self.makeTemporaryPermanent()
            status = ENTAILMENT
    ...
    def addAxiom(self, form, num, getprocess=False):
        if getprocess:
            if form == AtomTrue:
                return True
            if form == AtomFalse:
                return False
            deriv = Derivation(form=form, children=[], cost=0, derived=False, num=num)
            if not self.addDerivation(deriv):
                return False
            else:
                return True
        assert getprocess is False
    ...
    def is_unit(self,deriv):
        form=deriv.form
        if form == AtomTrue:
            return False
        return form.isa(Atom) or form.isa(Not)

    def ischild(self,deriv,deriv1):
        if len(deriv1.children)==0:
            return False
        if deriv1.children[0]==deriv or deriv1.children[1]==deriv:
            return True
        return self.ischild(deriv,deriv1.children[0]) or self.ischild(deriv,deriv1.children[1])

    def refreshcost(self,deriv,deriv1):
        if len(deriv1.children)==0:
            return 0
        if deriv1.children[0] == deriv:
            return max(deriv.cost,self.refreshcost(deriv,deriv1.children[1]))+1
        elif deriv1.children[1] == deriv:
            return max(deriv.cost, self.refreshcost(deriv, deriv1.children[0])) + 1
        else:
            return max(self.refreshcost(deriv, deriv1.children[0]),self.refreshcost(deriv, deriv1.children[1])) + 1

    def refresh(self,deriv):
        for key1, deriv1 in list(self.derivations.items()):
            if self.ischild(deriv,deriv1):
                deriv1.cost=max(self.refreshcost(deriv,deriv1.children[0]),self.refreshcost(deriv,deriv1.children[1]))+1

    # Return whether the Derivation is consistent with the KB.
    def addDerivation(self, deriv):
        # Derived a contradiction
        #print(deriv)
        if deriv.form == AtomFalse:
            self.derivations[AtomFalse]=deriv
            return False

        key = deriv.form
        oldDeriv = self.derivations.get(key)
        maxCost = 20
        if deriv.cost <= maxCost and (oldDeriv == None or oldDeriv.cost>deriv.cost):
            # Something worth updating
            if oldDeriv!=None:
                self.refresh(oldDeriv)
            ...
            while flag==0:
                ...
                for key2, deriv2 in list(self.derivations.items()):
                    if self.is_unit(deriv2):
                        if deriv2 in used:
                            continue
                        if not self.applyBinaryRules(deriv, deriv2):
                            return False
                        if not self.applyBinaryRules(deriv2, deriv):
                            return False
                        flag=0
                        used.append(deriv2)
    ...
    def applyUnaryRules(self, deriv):
        for rule in self.rules:
            if not isinstance(rule, UnaryRule):
                continue
    ...
    def applyBinaryRules(self, deriv1, deriv2):
        for rule in self.rules:
            if not isinstance(rule, BinaryRule):
                continue
            if rule.symmetric() and str(deriv1.form) >= str(deriv2.form):
                continue
            for newForm in self.ensureFormulas(rule, rule.applyRule(deriv1.form, deriv2.form)):
                if not self.addDerivation(Derivation(newForm, children = [deriv1, deriv2], cost = max(deriv1.cost,deriv2.cost) + 1, derived = True)):
                    return False
        return True
