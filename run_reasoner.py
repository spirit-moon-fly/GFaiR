...
class ConvertorDataset(Dataset):
    ...
    def __init__(self, args,split='train'):
        input_direc = os.path.join(args.input_dir,split+'_input_reasoner.pkl')
        output_direc = os.path.join(args.input_dir,split + '_output_reasoner.pkl')
...
def test(args, model, test_dataloader, device,tokenizers=None):
    model.eval()
    ...
    with open(args.output_dir + '/test_result_recording.txt', 'a+') as writer:
        ...
        writer.write(f'total_acc/total_num: {eval_accuracy}/{nb_eval_examples}\n')
...
def main():
    ...
    parser.add_argument("--output_dir",
                        default='out_reasoner/rclause_16_20_21',
                        type=str,
    ...
    if args.do_test:
        ...
        test(args, model, test_dataloader, device,tokenizers=tokenizer)
        exit()
    ...
    if args.do_train:
        ...
        optimizer=Adafactor(optimizer_grouped_parameters,lr=args.learning_rate,eps=(1e-30, 1e-3),clip_threshold=1.0,decay_rate=
